...
Full kram uses monotonic timed caches. The least complicated version of full KRAM caches the last request from any client inside a sliding time window. A new request must both have a later datetime than the cached request and must be inside the host time window. Requests that move outside the sliding time window may be safely deleted thereby pruning the cache. The timed cache can be made more granular by caching a request per message type for a given client or even more granular by caching messages with a specific time window for the message type for a given client or even more granular by caching messages for a given transaction type for a transactioned message type for a given client.   Essentially, creating an expiration datetime as the request datetime plus the host configured timewindow for all instances of a given message at a given stage in a transaction of a given type. The monotonicity of the cached requests protects against replay attacks. The time window bounds the size of the cache. Given the window size is determined by the transaction type, then one could also have a cache per transaction ID. The default window size cannot be per transaction ID as this enables a cache prune based replay attack for pruned transaction ID cache.

That has not been implemented yet in keripy. The time window needs to be controlled by the host relative to the host's clock otherwise the client can attack it.

With a host-controlled sliding window timed monotonic cache, because the monotonicity of the cache protects against a replay attack and the time window merely bounds the memory requirements, one can tune the window per message type (and per AID) and/or per transaction type, so that messages associated with certain message types or transaction can have relatively long time windows (days or weeks) which windows are only bounded by memory and the traffic load for those message from given clients.
...
When using per transaction ID caches the window size must be per transaction type not per transaction ID to avoid a cache prune replay attack. When the timeliness cache is granularized per transaction ID then interleaved messages from multiple transactions will not interfere with each other. 
...
Moreover, should messages include their own non-timeliness ordering, i.e., numeric ordinal, then both ordinals, timeliness, and numeric, can be employed in concert to better account for asynchronous networks that deliver messages out-of-order but that are not replay attacks. 
...
#### Full Kram MultiSig
For full KRAM with multisig, the window cache can be specific to a given transaction type, so a long enough window to collect signatures can be used. Moreover, with granular full KRAM each transaction type could employ a different acceptance policy.

The full KRAM approach can be modified to with work with group multi-sig by adding a partially signed message escrow that works in conjunction with the KRAM cache. This is described as follows:

A given message sourced by a multi-sig AID must be verifiably signed by at least one member of the group. If the message is the first received message, it must lie within the receiver's lagging time window with respect to its current time for that message type or for transaction messages for that transaction type. A new cache is created with the time stamp, message type and window based on message type or for transactioned messages the cached is based on messaged type transaction type, and transaction ID with a time window is determined by the transaction type. The message itself and its attached signatures are added to a partially signed escrow.

If the message is not the first message received for a given timestamp, message type for non transactioned message types or for message type, transaction type, and transaction ID for transactioned message types, but is the same as the cached one, then the message is accepted by full KRAM, and any attached and verified signatures are added to the partially signed escrow for that message. Later receipt of the same message ID but with different but verified member signatures is not considered a replay attack. The signatures are added to the database in an idempotent manner i.e., if a signature already exists, nothing changes. As long as neither the lagging time window nor the escrow timeout expires while collecting a threshold-satisficing number of signatures, the message will be accepted and moved out of escrow.
