...
A decentralized self-identifying data item is identified by a decentralized universally unique self-certifying identifier (DID). Self certifying means that the identifier includes either a public key or a fingerprint of a public key from a cryptographic public/private key pair. The DID is included as the value of a field in the data item itself. The data item also includes a field whose value is the DID for a signer of the data item. This may or may not be the same DID used for the data item itself. Attached to the data item is a signature that is verifiable as being generated by the private key associated with the public key in signer field's DID value. This signature verifies that the data item with its encapsulated identifier or identifiers were created by the holder of the associated private key for the signer.  The whole data item is self-identifing and self-certifying because all identifiers are encapsulated in the signed data are are verifiable against the privates keys associated with the public keys in the DIDs.
