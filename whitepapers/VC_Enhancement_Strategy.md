...
Interestingly enough an LPG can be implemented with URIs if one uses the query string to provide the edge properties and also uses JSON at the resource end point to provide the node properties. Image if Time Berners Lee has used that instead of RDF. Maybe the semantic web would still be a thing =).
...
In general, any knowledge graph database may be dumbed down into an RDF representation in order to fit the extreme constraints of the semantic web. But that dumbing down removes much of the expressive power of the the not dumbed down graph. JSON-LD/RF is relatively poorly suited for any other purpose but the semantic web. As it now stands almost nobody is actually using the open semantic web in any meaningful way.  Most if not all of the other open standard graph database formats are more powerful, more performant, and more popular than RDF. They all enable semantic interoperability by consuming JSON schema and more importantly provide more powerful, more convenient, and more performant semantic inference than RDF. 
