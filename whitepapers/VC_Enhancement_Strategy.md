...
The purpose of this write up is to better memorialize my increasingly worrisome concerns about the adoption path for Verifiable Credentials. My concerns started with the security limitations of VCs that use linked data (JSON-LD/RDF) but now extend to semantic inference limitations of linked data as well. A VC is a member of a more general class of data that may be described to as securely attributed or provenanced data. Other terms for describing this class of data include authentic data and more specifically provenanced data.  My overriding interest for this class of data is to reach and end state where we have universal adoption of securely attributed data. This fixes the broken internet. A a result the underlying technology and adoption strategy must be compatible with the end state. We may never reach that end state but my goal is that it won't be because of any technical reason nor because of a bad adoption strategy.
