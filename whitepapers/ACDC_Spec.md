...
### Schema SAID Field Label

Normally the convention is that the field label for SAID fields is `d`. In the case of the schema section, however, the field label is `$id`. This repurposes the JSON Schema defined id field label, `$id`.  The field value of field `$id` is meant to be the unique identifier of the schema instance. In ACDC we repurpose that field to provide the verifiable SAID of the schema as a SAD. This enables verifiable immutability on the schema so identified.
The value of the top-level schema, `s`, field may be either the schema itself for the SAID from the schema's top-level `$id` field.


### Static Schema

For security reasons, the full schema of an ACDC must be completely self-contained and statically fixed (immutable) for that ACDC. By this, we mean that no dynamic schema references or dynamic schema generation mechanisms are allowed. The serialization of a static schema may be self-contained. A compact commitment to the detailed static schema may be provided by its SAID. In other words, the SAID of a static schema is a verifiable cryptographic identifier for its SAD. In essense, compliant schema in ACDC must be SADs. In other words, they are therefore SAIDified. The associated detailed static schema (uncompacted SAD) is cryptographically bound and verifiable to its SAID. The detailed static schema may be cached or attached. But composed, cached, and attached static schema are not to be confused with dynamic schema. Nonetheless, while securely verifiable, a remotely cached SAIDified schema resource may still have availability problems. Availability is a separate concern. ACDCs are designed to be end-verifiable when available.  
